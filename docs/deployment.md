...
| Component | Host | Role |
| --------- | ---- | ---- |
| Frontend (`frontend/`) | **Vercel** | Static Vite build, SPA routing, `/api` proxy to backend |
...
The browser calls the **Vercel origin** at `/api/...`. Vercel middleware forwards those requests to the Railway backend (same pattern as Docker/nginx locally).
...
3. Vercel reads `frontend/vercel.json`:
   - **Build command:** `npm ci && npm run build`
   - **Output directory:** `dist`

### Connect to Railway

The axios client uses `VITE_API_URL` when set, otherwise **`/api`** (see `frontend/src/shared/http/client.ts`).
...
3. Leave `VITE_API_URL` unset so the browser keeps calling `/api` on the Vercel origin.

**Alternative — direct Railway URL**

Set `VITE_API_URL` to your Railway domain at build time. The browser calls the backend directly; you must configure CORS and cookie `SameSite` carefully. See [Production hardening](./production-hardening.md).
...
flowchart LR
    Browser["Browser"]
    Vercel["Vercel<br/>React SPA + /api proxy"]
...
This mirrors the local stack (nginx/Vite proxy → backend → Postgres) described in the [README](../README.md#-architecture).
...
| 401 after login | Cookie not sent cross-origin | Use `/api` proxy on Vercel (do not set `VITE_API_URL`) |
